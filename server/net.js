...
import { byHandle, wake } from './agents.js';
...
const handleOf = (id) => id ? (one('SELECT handle FROM agents WHERE id=?', id)?.handle || '?') : 'system';
...
/** Send a message. target: "@handle" (DM) or "#channel". sender=null → system. */
export function sendMessage(sender, target, text, { reply_to = null, system = false } = {}) {
  ...
  if (target.startsWith('@') || (!target.startsWith('#') && byHandle(target))) {
    ...
    const r = run('INSERT INTO messages(channel,from_agent,to_agent,content,reply_to,created_at) VALUES(?,?,?,?,?,?)',
      'dm', sender?.id, to.id, content, reply_to, now());
    wake(to.id, 20_000);
    emit('dm', sender?.id, `✉️ @${sender?.handle || 'system'} → @${to.handle}: ${trunc(content, 140)}`, { id: Number(r.lastInsertRowid) });
  ...
  const r = run('INSERT INTO messages(channel,from_agent,content,reply_to,created_at) VALUES(?,?,?,?,?)', ch.slug, sender?.id, content, reply_to, now());
  ...
  emit('message', sender?.id, `${icon} #${ch.slug} @${sender?.handle || 'system'}: ${trunc(content, 160)}`, { id, channel: ch.slug });
  return { id, content };
}
...
export function unreadMentions(agent, limit = 8) {
  ...
  const rows = all("SELECT * FROM messages WHERE channel!='dm' AND id>? AND hidden=0 AND content LIKE ? AND from_agent!=? ORDER BY id DESC LIMIT 50",
